...
BEGIN TRANSACTION
	...
	BEGIN
	RAISERROR ('É CITYCOL, DELETANDO APENAS AS INUTILIZAÇÕES REJEITADAS',0,1) 
	BEGIN TRANSACTION		
		WHILE (@Deleted_Rows > 0)
			BEGIN
...
USE [NFE]
GO
RAISERROR ('Iniciando processo de compactação do banco.',0,1)  
...
RAISERROR ('Iniciando compactação do banco de dados NFe.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 

DBCC SHRINKDATABASE(@DBNAME, TRUNCATEONLY)

RAISERROR ('Compactação realizada.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 
...
-----------------------------
RAISERROR ('Iniciando compactação do arquivo NFe.mdf.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 
DBCC SHRINKFILE (@DBNAME , TRUNCATEONLY)
RAISERROR ('Compactação realizada no arquivo NFe.mdf.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 

RAISERROR ('Iniciando compactação do arquivo NFe.ldf.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 
DBCC SHRINKFILE (@LOGNAME , TRUNCATEONLY)
RAISERROR ('Compactação realizada no arquivo NFe.ldf.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 
PRINT N'=================================================='
RAISERROR ('Processo de compactação terminado.',0,1)  
PRINT N''+RTRIM(CAST(GETDATE() AS NVARCHAR(30))) 
